...
class UpdateAction
{
    ...
    public function execute(ReleaseTypeDTO $dto, int $id)
    {
        /** Поиск записи по id, в Случае ошибки выкинет исключение 404 */

        $rt = $this->dbService->update($this->model, $id, $dto);
        $oldData = $rt['oldData'];
        $rt = $rt['instanceModel'];

        /** Проверка на изменения данных модели */
        $newData = $rt->getChanges();
        ...
        if ($newData) {
            foreach ($newData as $field => $value) {
                ...
                Log::channel('change')
                    ->info(
                        message: 'Запись ReleaseType была изменена',
                        context: [
